 ...
 * @LastEditTime: 2022-06-21 19:36:59
...
func CreatePmTag(pmTag models.PmTag) int64 {
	orm := orm.NewOrm()
	node, err := utils.NewWorker(1)
	Id := node.GetId()
	res, err := orm.Raw("insert into pm_tag set id = ?,name = ?,url = ?", Id, pmTag.Name, pmTag.Url).Exec()
	if err == nil {
		//返回执行成功条数
...
func SelectPmTag() []models.PmTag {
	orm := orm.NewOrm()
	var list []models.PmTag
	res, err := orm.Raw("SELECT * FROM pm_tag").QueryRows(&list)
	if err == nil {
		//返回执行成功条数
